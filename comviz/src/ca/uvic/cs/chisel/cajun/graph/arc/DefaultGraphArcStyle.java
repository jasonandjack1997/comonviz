...
public class DefaultGraphArcStyle implements GraphArcStyle {
	...
	public Paint getTypePaint(Object type) {
		...
		if (paint != null) {
			return paint;
		} else if ((type != null) && arcTypeToColor.containsKey(type)) {
	...
	/**
	 * Sets the arc types - this is done to map a background color/gradient to
	 * each arc type.
	 * 
	 * @param arcTypes
	...
	/**
	 * Adds the arc type and maps a color/gradient to it which is used as the
	 * line color for the arc. If the arc type already exists then nothing is
	 * done.
	 * 
	 * @param type
	...
	/**
	 * Returns the paint to use for the arc. Checks if the arc type has a color
	 * mapped to it, if so that color/paint is returned. Otherwise the default
	 * arc color is returned.
	 * 
	 * @param arc
...
//		return createStroke(width, isDashed);
		...
		return StyleManager.getStyleManager().getArcStroke(arc);
	}
	...
	protected Stroke createStroke(float strokeWidth, boolean dashed) {
		// any point in caching these instead of creating them every time a
		// paint occurs?
		Stroke stroke;
		if (dashed) {
			...
			try {
				stroke = new PFixedWidthStroke(strokeWidth, capSquare,
